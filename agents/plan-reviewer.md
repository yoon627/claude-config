...
14. **`# Intent`** — plan 파일이 있는 medium 이상 계획(메인이 호출 시 전달한 dlc 규모 기준 — 미전달이면 검사 생략)이면 `# Intent` 가 있어야 한다(`skills/dlc/SKILL.md` 요구사항 명확화). 단 이 규칙(2026-09-09) 이전에 만들어진 plan·이어받는 plan 은 소급 대상이 아니고, 계획 텍스트만 전달된 경우는 검사하지 않는다. 있으면 반박한다: 사용자 확인 근거가 없는 내용이 ⚠️ 없이 확정처럼 쓰였나 · Constraints 에 **이 작업 고유의** 제약(§0·§1·worktree 같은 기본 제약이 아닌 것)이 빠졌나 · `없음 — <근거>` 의 근거가 성립하나(맨 `없음`·성립하지 않는 근거는 지적). **규모와 무관하게** frontmatter `intent:` 가 있으면 그 `intent.md` 를 먼저 read 한 뒤 합성된 요구로 검사하고, plan `# Intent` 가 링크+델타만인지(공통 제약 복제 없음)를 §10 묶음 intent 규약대로 본다.
