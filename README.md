...
- `permissions.ask` — **비어 있음**. `git push` 확인 4건이 있었으나 auto 모드 무마찰 요구(2026-08-06 사용자 지시)로 제거 — `ask`/`deny` 규칙은 권한 모드보다 우선해서, 남겨두면 auto 모드에서도 매번 프롬프트가 뜬다. 이제 push 는 안전 분류기 판정에 맡긴다. 다른 repo 의 main 푸시는 위 `pre-push` 훅이 계속 하드 차단하므로 보호는 유지된다.
