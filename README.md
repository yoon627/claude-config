...
- `permissions.deny` — **비어 있음**. `git push origin main/master` 직접 푸시 차단이 있었으나 이 repo(`~/.claude`) main push 허용(2026-08-05 사용자 승인)으로 제거 — deny 는 프로젝트 단위 범위 지정이 불가하고 allow 보다 우선하므로, 전역 deny 를 남기면 이 repo 도 막힌다. 다른 repo 의 main 푸시는 **그 repo 에서 `install-hooks` 를 실행했을 때만** git `pre-push` 훅이 하드 차단한다(설치는 repo 별 opt-in, 전역 `core.hooksPath` 없음 — 2026-09-25 감사에서 `~/Repos` 6개 중 설치 0곳 확인. `pre-commit-check` — repo 루트가 `~/.claude` 일 때만 면제). 즉 훅을 설치한 repo 만 이 레이어가 보호하고(repo 로컬 `core.hooksPath` 가 있거나 linked worktree 에서 설치하면 그마저 무효 — intent G5), 나머지 repo 는 CLAUDE.md §8 규약과 아래 `ask` 에만 의존한다.
