...
- **`collect-state.sh`** (헬퍼): 마무리 2단계·6단계의 읽기전용 git 신호(worktree 위치·dirty·upstream/unpushed·base merged·**로컬 default merged**(`localDefault`·`mergedToLocalBase`)·ignored)를 평문 `key:value` 로 1회에 수집 — 분산된 개별 git 호출의 왕복을 줄인다. read-only(판정·삭제·파괴 명령은 SKILL 메인), 각 점검 fail-safe(실패 필드 none/unknown), `unpushedStatus` 는 false 와 unknown 을 구분해 false-positive 삭제를 막는다.
