...
- **16 Report — feedback memory 판정(§12)**: 작업 중 **사용자 교정·리뷰 지적**이 있었으면 §12 feedback memory 저장을 *판정*한다 — 대상이면 memory 파일 + `MEMORY.md` 인덱스 한 줄(둘 다), 비대상이면 사유 1줄. 저장 강제가 아니라 **판정 누락 금지**(wiki ingest 판정과 대칭 — 사용자 리뷰가 다음 구현에 반영되는 loop 의 5단계).
...
- **simplify 체크(13단계)**: 격리 subagent 가 아니라 **메인 직접** — 모든 spoke 는 read-only. diff 범위에서 점검: 중복(3회+ 반복만 추상화 검토) · 과한 추상화 · 불필요 옵션/죽은 분기 · 죽은 코드(rg 사용처 확인 후 삭제) · 과한 방어 코드 · 표준 라이브러리/기존 유틸로 대체 · 깊은 nesting. 동작 보존 · 요청 범위 내 · 불확실하면 보류(제안만 Report 에). substantive 수정 시 targeted test + 14 targeted 재리뷰 필수.
...
- **자동 신호 수집과의 관계**: hook(`scripts/dlc-signal.js`)이 early-stop·doc-drift·guard 차단·plan-blocked 등 발동 신호를 `~/.claude/telemetry/dlc-signals.jsonl` 에 자동 누적하고, `/improve` 가 집계·분석해 개선 후보를 랭킹 제안한다. 이 절의 수동 기록은 신호가 못 담는 **맥락**(원인·재발 조건·수정 후보 위치)을 남기는 것 — 자동 수집과 상보이며 서로 대체하지 않는다.
