...
- **16 Report — recap+선택지로 닫기(CLAUDE.md §3-6)**: 증거 게이트 통과 후, 보고는 **결론 요약(≤3줄, 무엇이 끝났고 status)을 먼저** 내고 이어서 **선택지를 AskUserQuestion** 으로(작업 확인 / 마무리·정리(push·PR·머지·worktree 정리→아래 정리 판정) / 다른 작업 이어가기 / 종료 — 큰·낯선 변경이면 "변경 이해 리포트+퀴즈" 옵션 추가). 사용자가 이미 다음 지시를 준 흐름이면 선택지 생략(중복 질문 금지). recap 은 보고 형식일 뿐 16단계 표에 새 단계를 더하지 않는다.
