...
- **체크리스트** — ① 완료 기준 ② 대상 파일/기능 범위 ③ 사용자-visible 산출물·문구 형태 ④ 명시적 금지/제외 범위. 빈 칸이 **완료 판정(acceptance)을 바꾸면 무조건 질문**(`AskUserQuestion`). 빈 칸이 없거나 공백이 결과무관이면 **합리적 가정 + 그 가정 1줄 명시** 후 진행.
