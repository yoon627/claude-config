...
비trivial 변경인데 **현재 작업 worktree 가 아니면**(main worktree 이거나 worktree 밖) dlc 는 직접 구현 전 `wt` 로 worktree 를 만들어 그 안에서 진행한다.

- **판정**: `git worktree list --porcelain` 첫 worktree(=main) 와 현재 cwd 비교. 현재가 main worktree 면 → wt 경로.
...
- **이중 보조**: Stop hook(`early-stop-guard`)이 "비trivial 변경 있는데 검증 기록 없음"을 capped 로 경고한다(fail-open). hook 은 보조일 뿐 — 이 규약이 단일 소스다.
...
## Workflow Findings (증거기반 자기개선 — 최소형)

작업 중 **확인된 workflow 실패**(dlc/규약 자체의 문제로 작업이 샌 경우)만 기록한다 — 매 작업 회고가 아니다(빈 의례 방지). 자동 수정은 하지 않는다.

- **기록 트리거**(확인된 것만, 둘 중 하나): ① 중대 self-diagnosis 발동(스코프 이탈·단계 누락·격리 위반 등) ② 동일 유형 실패 2회 재현.
- **기록 형식**: plan `# Workflow Findings` 에 한 줄 — `깨진 규칙/단계 · 재발 조건 · 수정 후보 위치(어느 SKILL/규칙)`.
- **승격**: 같은 finding 이 2회 누적되면 dlc SKILL 또는 wiki `decision` **수정 제안**으로 Report 에 올린다. 실제 수정은 **사용자 승인 후 별도 작업** — 운영 자산 자가 수정 금지(CLAUDE.md §1).
- **한계**: 실수의 *발견·기록*까지가 harness 몫. "스스로 규약을 고치는" 완전 자동화는 capability 영역이라 두지 않는다(`wiki/pages/decision/self-diagnosis-and-improvement-status.md` 결정).
