...
### 2. 작업 상태 수집 + 임시 커밋
- **수집 (`collect-state.sh` 1회)**: `bash skills/e/collect-state.sh` 로 읽기전용 신호를 평문 `key: value` 로 받는다 — `root·branch·detached·mainWorktree·dirty·status·upstreamStatus·upstream·unpushedStatus·unpushed·base·baseValid·inBase·patchInBase·remoteContainingHead·ignoredStatus·ignored`(개별 git 10+ 호출을 1회로 묶음). 각 라인은 **첫 `: ` 1회로만 split**(경로 `C:/`·커밋 subject 의 `:` 보존), list 필드(`status`·`unpushed`·`remoteContainingHead`·`ignored`)는 들여쓰기 라인. `git log --oneline`(plan 시작 이후 새 커밋)은 별도. **헬퍼 실패·필드 누락이면** 보고에 명시하고 개별 git 명령으로 폴백(5단계 괄호 폴백). 점검 실패(origin 없음·status 오류)는 해당 필드 none/unknown(false 로 평탄화 안 함).
...
- **recap 형식(CLAUDE.md §3-6)**: 위 보고는 **결론 요약(≤3줄, 무엇이 끝났고 status)을 먼저**. **`/e` 호출 자체가 "마무리" 지시**이므로 §3-6 예외(사용자가 이미 다음 지시를 준 흐름 → 선택지 생략)에 따라 **새 AskUserQuestion(4선택지)을 만들지 않는다**(마무리 시점 1회 원칙). 마무리 액션은 아래 5단계 worktree 정리 제안(조건 충족 시)과 6단계 "다음 세션 `/c`" 안내가 담당한다 — step5 는 조건부(done·plan-in-worktree)라 안 뜰 수 있고 그때도 별도 질문을 새로 만들지 않는다.

### 5. worktree 정리 제안 (조건부)
마무리가 끝난 뒤, 현재 worktree 가 **역할을 다했고 안전하게 지울 수 있으면** 삭제를 제안한다 — **자동 삭제 안 함, 항상 AskUserQuestion**.
- **제안 조건 (모두 충족 시에만)**: 2단계 이후 WIP 커밋·plan write 로 상태가 바뀌므로 **삭제 직전 `bash skills/e/collect-state.sh` 를 한 번 더 실행**해 그 신호로 판정한다(2단계 스냅샷 재사용 금지 — 재수집 invariant). **헬퍼 실패·필드 누락·파싱 불가면 제안 생략(보수)**. 괄호는 헬퍼 불가 시 폴백 명령:
  1. cwd 가 **비-메인 worktree** — `root` ≠ `mainWorktree`(폴백: `git rev-parse --show-toplevel` vs `git worktree list --porcelain` 첫 `worktree <path>`). 슬래시 방향·대소문자 normalize 후 비교(이 비교는 메인 책임 — 헬퍼는 raw path 만). 메인이면 제안 안 함.
  2. **`detached`=false** + plan `status == done`(이번 4단계 확정 — plan 의존이라 헬퍼 밖). detached 면 제안 생략.
  3. **working tree 변경 없음**(untracked 포함) — `dirty`=false(`status` 빈; 폴백 `git status --porcelain`). `dirty`=unknown(status 명령 실패)이면 점검 불가 → 제안 생략(보수).
  4. **unpushed 없음** — `unpushedStatus` ∈ {upstream, allRemotes} 이고 `unpushed`=(none)(폴백: upstream 있으면 `git log @{u}..HEAD`, 없으면 `git log <branch> --not --remotes`; `git log origin/<branch>..HEAD` 는 ref 부재 시 fatal 이라 안 씀). **`unpushedStatus`=unknown(원격/upstream 불명)이면 미보존으로 보고 제안 안 함** — false-positive 삭제 차단.
  5. **base 또는 다른 원격 브랜치에 merged 됨 (둘 다 git-only 추정 — 확정 아님)** — pushed(조건4)는 원격 보존일 뿐, PR 리뷰 중·머지 전이면 지우면 안 된다. 아래 (a)·(b) 중 하나라도 머지 신호면 제안하되 확정이 아니며 최종 삭제는 늘 AskUserQuestion:
     - **(a) 기본 base 포함**: `baseValid`=true 이고 (`inBase`=true **또는** `patchInBase`=true) 면 base 에 merged. `inBase` 는 커밋 SHA 포함(fast-forward/no-ff 머지), `patchInBase` 는 patch-equivalent 포함(`git cherry` — **rebase merge 로 SHA 가 재작성돼도** 브랜치 전 커밋이 base 에 patch 로 있으면 감지). `baseValid`=false(원격/기본브랜치 미설정)면 이 신호 skip. (폴백: `BASE`=`git symbolic-ref --short refs/remotes/origin/HEAD`||`origin/main`, `git rev-parse --verify --quiet <BASE>` 후 `git log <BASE>..HEAD` 빈지[inBase]; 비어있지 않으면 `git cherry <BASE> HEAD` 의 `+`(미머지 patch)가 0개인지[patchInBase].)
     - **(b) 다른 원격 브랜치 포함 (ticket-to-ticket 머지 감지)**: `remoteContainingHead` 에서 self(`origin/<현재브랜치>`)와 `origin/HEAD -> ...` 라인을 제외하고 남는 원격 브랜치가 있으면 거기 머지됐을 가능성(이 worktree 가 다른 ticket 위로 분기·PR 된 경우). ⚠️ **진행 중인 다른 브랜치가 내 커밋을 조상으로 쌓은 경우도 포함**되니 확정이 아니다 → 그 브랜치명을 근거로 AskUserQuestion 본문에 "`<HEAD7>` 가 `<remote-branch>` 에 포함됨 — 머지됐을 수 있으나 진행 중 브랜치일 수도(확정 아님)" 명시. (폴백: `git branch -r --contains HEAD`.)
     - (a)·(b) 모두 신호 없으면(self 만 남거나 BASE 미포함) → 미머지로 보고 생략·유지.
     - ⚠️ 한계: **rebase merge**(SHA 재작성, 커밋 1:1)는 이제 `patchInBase`(git cherry patch-id)가 (a)에서 감지한다. 다만 **squash merge**(N커밋→1)는 patch-id 가 합쳐져 cherry 도 못 잡아 미머지로 보이고, stale 원격 ref·rebase 중 충돌 해소로 patch 가 바뀐 경우도 그러함 — 전부 유지 방향이라 안전. PR 상태 직접 조회는 인증·headless 때문에 안 쓴다 — git-only 추정 + 사용자 확인. 1순위는 false-positive(미머지 삭제) 차단이라 (b)·`patchInBase` 는 "확정"이 아니라 "근거 제시"로만(최종 삭제는 늘 AskUserQuestion).
  6. **미보존 산출물 점검 (필수)** — `git worktree remove`(--force 없이)는 **gitignored 파일을 경고 없이 함께 삭제**하고 `--force` 는 미커밋 산출물도 지운다. whitelist `.gitignore` 라 `.env` 등 gitignored 는 `git status` 엔 안 보인다(plans/ 는 tracked §10 라 미커밋 plan 은 보이고 remove 가 거부하지만 `--force` 로는 유실 가능). **`ignoredStatus`=unknown(점검 명령 실패)이면 제안 생략(보수)**. `ignored` 목록(폴백 `git status --porcelain --ignored`) + `git status --short` 로 점검:
     - 이 worktree `plans/` 에 **이번에 갱신한 미커밋 plan 이 있으면 → 제안 생략**(방금 done 기록한 plan 이 worktree 와 함께 소실 방지; 브랜치에 commit·push 하거나 main worktree `plans/` 로 옮긴 뒤에야 안전).
...
- **대상**: 5단계 후에도 세션이 **비-메인 worktree** 에 있을 때(`--show-toplevel` ≠ main path, 정규화 후 비교). 메인이면 skip.
