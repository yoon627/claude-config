...
- **제안 조건 (모두 충족 시에만)**: 2단계 이후 WIP 커밋·plan write 로 상태가 바뀌므로 **삭제 직전 `bash skills/e/collect-state.sh` 를 한 번 더 실행**해 그 신호로 판정한다(2단계 스냅샷 재사용 금지 — 재수집 invariant). **헬퍼 실패·필드 누락·파싱 불가면 제안 생략(보수)**. 괄호는 헬퍼 불가 시 폴백 명령:
  ...
  6. **gitignored 산출물 점검 (필수)** — `git worktree remove`(--force 없이)는 **gitignored 파일을 경고 없이 함께 삭제**한다. whitelist `.gitignore` 라 `plans/`·`.env` 가 ignored → `git status` 엔 안 보인다. **`ignoredStatus`=unknown(점검 명령 실패)이면 제안 생략(보수)**. `ignored` 목록(폴백 `git status --porcelain --ignored`)으로 점검:
