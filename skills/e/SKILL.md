...
### 2. 작업 상태 수집 + 임시 커밋
- **수집 (`collect-state.sh` 1회)**: `bash skills/e/collect-state.sh` 로 읽기전용 신호를 평문 `key: value` 로 받는다 — `root·branch·detached·mainWorktree·dirty·status·upstreamStatus·upstream·unpushedStatus·unpushed·base·baseValid·inBase·remoteContainingHead·ignoredStatus·ignored`(개별 git 10+ 호출을 1회로 묶음). 각 라인은 **첫 `: ` 1회로만 split**(경로 `C:/`·커밋 subject 의 `:` 보존), list 필드(`status`·`unpushed`·`remoteContainingHead`·`ignored`)는 들여쓰기 라인. `git log --oneline`(plan 시작 이후 새 커밋)은 별도. **헬퍼 실패·필드 누락이면** 보고에 명시하고 개별 git 명령으로 폴백(5단계 괄호 폴백). 점검 실패(origin 없음·status 오류)는 해당 필드 none/unknown(false 로 평탄화 안 함).
...
- **제안 조건 (모두 충족 시에만)**: 2단계 이후 WIP 커밋·plan write 로 상태가 바뀌므로 **삭제 직전 `bash skills/e/collect-state.sh` 를 한 번 더 실행**해 그 신호로 판정한다(2단계 스냅샷 재사용 금지 — 재수집 invariant). **헬퍼 실패·필드 누락·파싱 불가면 제안 생략(보수)**. 괄호는 헬퍼 불가 시 폴백 명령:
  ...
  5. **base 또는 다른 원격 브랜치에 merged 됨 (둘 다 git-only 추정 — 확정 아님)** — pushed(조건4)는 원격 보존일 뿐, PR 리뷰 중·머지 전이면 지우면 안 된다. 아래 (a)·(b) 중 하나라도 머지 신호면 제안하되 확정이 아니며 최종 삭제는 늘 AskUserQuestion:
     - **(a) 기본 base 포함**: `baseValid`=true 이고 `inBase`=true 면(브랜치 커밋이 모두 `base` 에 포함) base 에 merged. `baseValid`=false(원격/기본브랜치 미설정)면 이 신호 skip. (폴백: `BASE`=`git symbolic-ref --short refs/remotes/origin/HEAD`||`origin/main`, `git rev-parse --verify --quiet <BASE>` 후 `git log <BASE>..HEAD` 빈지.)
     ...
     - ⚠️ 한계: squash/rebase 머지는 원커밋 SHA 가 어느 브랜치에도 안 남아 (a)·(b) 둘 다 못 잡아 미머지로 보이고(no-ff 전제), stale 원격 ref 도 그러함 — 둘 다 유지 방향이라 안전. PR 상태 직접 조회는 인증·headless 때문에 안 쓴다 — git-only 추정 + 사용자 확인. 1순위는 false-positive(미머지 삭제) 차단이라 (b)는 "확정"이 아니라 "근거 제시"로만.
