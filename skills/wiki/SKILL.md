...
- 구조 점검(dead link·orphan·outbound<2·index 동기화·frontmatter)은 `check_links.py`(`uv run --no-project python "${CLAUDE_SKILL_DIR}/check_links.py"`), 의미 점검(모순·stale)은 LLM.
