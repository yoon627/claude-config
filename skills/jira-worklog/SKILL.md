...
`/e` 는 6단계에서 이 스킬을 호출하며, 승인 여부는 `skills/e/SKILL.md` 6단계가 정한다(`/e` 호출 자체가
등록 동의라 별도 승인 질문을 만들지 않는다). 그 밖의 경로에서 호출하면 먼저 dry-run 으로 시간을 보여주고,
사용자 승인 후 `--register` 로 등록한다(외부 반영).
