...
# 실제 Jira 등록 (그날 항목 upsert — 같은 날 재실행 시 시간 갱신)
...
- **upsert(멱등)**: (티켓, 날짜) 마커로 그날 **본인** worklog 를 찾아 없으면 생성, 있으면 시간만 갱신한다(재실행해도 중복 없음). 갱신 시 기존 comment 는 보존. 타인의 worklog(같은 마커·다른 author)는 건드리지 않는다.
