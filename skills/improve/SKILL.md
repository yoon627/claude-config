...
- 점검 7(신규): `~/.claude/telemetry/dlc-signals.jsonl` 집계 — kind 별 `sessions`(unique)·`raw`·기간. **failure 축**(early-stop-verify·doc-drift-*·guard-worktree-deny·plan-blocked)과 **activity 축**(router-*·review-disposition — 실패 아님, 활동량)을 분리 표시. 효과 판단은 sessions(unique) 우선 — raw 는 같은 세션 반복 발동에 지배될 수 있다.
