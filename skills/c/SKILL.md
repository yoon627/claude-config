...
- **그 다음 이어서 실행** — `# Next` 가 명확하면 "진행해도 되나?" 재확인 없이 곧장 진행한다. 코드/파일을 바꾸는 구현이면 규모 불문 dlc 파이프라인으로 넘기며, **dlc 가 main 밖이면 wt-first 게이트를 적용하므로 worktree 격리는 그대로 유지**된다(우회 아님 — 다만 worktree 생성 자체는 로컬·가역이라 확인 없이 만들고 되돌릴 정보를 보고한다, §1 위험기반 승인). **단 아래 예외면 멈추고 사용자 결정을 기다린다 (닫힌 목록 5종)**:
  ...
  5. `done` plan — 이어갈지 확인. 단 `# Next` 에 "`/e merge` 재실행"(머지 대기·결과 불명 — `/e` 머지 모드의 QUEUED/UNKNOWN)이 적혀 있으면 완료가 아니라 **머지 대기 중** — 그 재실행을 안내한다(외부 쓰기라 예외 4 대로 확인).
