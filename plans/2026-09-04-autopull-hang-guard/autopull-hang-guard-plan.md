...
- 2026-09-04 **CI 2차 red — 리뷰어의 M4(dash 그룹 kill 미성립)가 CI 에서 확정됐다.** 테스트 `⑫`
  가 GitHub ubuntu 러너에서 실패(손자 생존). **로컬 WSL 은 10/10 통과해서 못 잡았고 CI 가 잡았다**
  — 로컬 Linux 재현이 CI 와 동치가 아니라는 실증.
  → 그룹 분리 수단을 플랫폼별로 고른다: **Linux `setsid`**(fork 없이 exec 하므로 `$!` 가 그대로
  새 세션 리더), **Git Bash `set -m`**(setsid 부재). **겹쳐 쓰면 안 된다** — `set -m` 하에선 배경
  job 이 이미 그룹 리더라 setsid 가 fork 하고 `$!` 가 래퍼를 가리켜 `wait` 가 조기 반환한다.
  WSL/dash 에서 기구 검증: fork 없음 · 그룹 kill 성공(폴백 불필요) · `wait rc=143` · 손자 사망.
