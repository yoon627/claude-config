...
리뷰를 **plan 단계**(구현 전)와 **code 단계**(구현 후)로 분리한 결정. 표준 순서: `plan-reviewer → 구현 → code-reviewer → simplify 체크(메인 직접) → 최종 검증`([[dlc-development-cycle]]).
...
- **simplify 체크**(통과 후 항상, 2026-07-04 부로 subagent 아닌 메인 직접): 중복·과한 추상화·죽은 코드 제거. 동작 보존, substantive 수정 시 targeted 재검증.
