...
- SessionStart pull 이 CI 를 통과한 커밋까지만 따라가게 하는 검증 게이트(CI 가 `ci/verified` 에 기록)는 [[autopull-verified-ff]].
