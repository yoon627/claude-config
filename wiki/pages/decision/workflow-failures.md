...
| worktree 격리 세션의 네이티브 Bash 가드가 `docs/codex-review.md` 정본 codex 호출(`--skip-git-repo-check`)·heredoc 본문의 git 언급·복합 원라이너를 거부 → §9 codex 병행이 worktree 에서 구조적으로 막힘 | 파일을 바꾸는 작업(=항상 worktree)에서 정본대로 codex 를 부를 때마다. 분류기가 비결정적이라 같은 형태가 통과/거부 갈림 | `docs/codex-review.md` §3 — 프롬프트를 파일로 분리하고 해당 플래그 제거한 형태로 정본 갱신 ([[worktree-isolation-bash-guard]]) | 3 | proposed (2026-09-02 — plan e-merge-path Deferred major) |
