...
| rtk 가 `tail -N f`·`cat a b c` 를 `rtk read -N f`·`rtk read a b c` 로 재작성 → `rtk read` 가 미지원 인자에서 서브커맨드명으로 폴백해 **`/usr/bin/read` 를 실행** → `read: -1: invalid option` / `not a valid identifier` | `tail -N`·다중 파일 `cat`/`head`/`tail` 이 든 Bash 호출마다(결정적) | **rtk 바이너리 업그레이드** — 설치본 0.28.2, 상류는 이 둘을 각각 **0.35.0**(`cmd: read/cat multiple file`)·**0.39.0**(`head/tail multi-file rewrite falls back to native command` #1362)에서 수정함. `hooks/rtk-rewrite.sh` 는 규칙 없는 얇은 위임자이고 `.gitignore` 로 untracked·rtk 관리라 **수정 위치가 아니다** | 6 | fixed (2026-09-02 확인 — headroom retire 와 함께 rtk 를 `~/.local/bin/rtk-0.44.2` 로 교체, 두 상류 수정 모두 포함. 삭제된 memory `rtk-headroom-path-fix` 가 가리키던 심링크 구성은 더 이상 유효하지 않다) |
| `dlc-task-router`(UserPromptSubmit) 가 subagent 완료 `<task-notification>` 턴에도 실행돼 알림 본문의 "재현·failing" 으로 `router-investigation` 오발동 + 그 턴의 `ledger.reset` 이 early-stop 의 changed/verified·doc-drift 판정을 삭제 | 리뷰어·subagent 결과 알림마다(결정적 — 한 세션 4회 실측) | `scripts/dlc-task-router.js` — `<system-reminder>`·`<task-notification>` 블록을 걷어낸 사용자 텍스트만 매칭, 남는 텍스트 없으면 리셋 없이 종료 ([[claude-code-hook-notification-turns]]) | 4 | fixed (2026-09-02, PR #149) |
| worktree 격리 세션의 네이티브 Bash 가드가 `docs/codex-review.md` 정본 codex 호출(`--skip-git-repo-check`)·heredoc 본문의 git 언급·복합 원라이너를 거부 → §9 codex 병행이 worktree 에서 구조적으로 막힘 | 비trivial 작업(=항상 worktree)에서 정본대로 codex 를 부를 때마다. 분류기가 비결정적이라 같은 형태가 통과/거부 갈림 | `docs/codex-review.md` §3 — 프롬프트를 파일로 분리하고 해당 플래그 제거한 형태로 정본 갱신 ([[worktree-isolation-bash-guard]]) | 3 | proposed (2026-09-02 — plan e-merge-path Deferred major) |
| repo `.claude/settings.json` 의 `type: agent` PreToolUse hook(`if: Bash(git commit*)`)이 `$`·backtick·heredoc 이 든 비커밋 명령에 발화해 code-reviewer 의 `codex exec` 병행 리뷰를 차단 → §9 codex 병행이 구현 단계에서 빠지고 pre-push 가 8라운드로 대신 잡음 | 커밋 아닌 Bash 에 `$VAR`·`$()`·heredoc 이 있을 때, 판정 비결정적(40건 차단 중 실제 commit 3건) | 프롬프트 0단계에 범위 자체 판정(`tool_input.command` 가 commit 아니면 `{"ok": true}`) ([[lesson-agent-hook-if-best-effort]]) | 37 | fixed (2026-09-03 — coin-trading-bot PR #165) |
...
| early-stop-verify 오탐 **sub-class(생태계 커버리지)** — `VERIFY` 정규식이 node/python/JVM 중심이라 컨테이너·셸·스타일·타 생태계 검증기를 못 알아본다. 사용자가 실제로 검증해도 `verified=false` 로 남아 종료 시 경고 | 2026-08-13 telemetry 조사: `.md` 제외 fix(07-17) 이후 발동 29건 중 8월 5건이 전부 `compose.yaml`×2·`security_workflow.css`·`settings.json`·`input_contracts.py`(타 repo), 7월엔 `smoke.sh`·`deploy.sh`·`pre-push`. 실측 — `docker compose config`·`stylelint`·`shellcheck`·`make lint`·`dotnet test`·`rspec`·`terraform validate`·`prettier --check` **전부 MISS**(`./gradlew test` 는 MATCH → `.kt` 14건은 이 class 아님) | `scripts/dlc-evidence-ledger.js` — `VERIFY` 를 `VERIFY_TOOLS`(그 자체가 검증) + `VERIFY_SUBCMD`(서브커맨드 필요)로 2분할해 확장. `docker compose up`·`terraform apply`·`prettier --write`·`black .` 은 계속 미인식(음성 테스트로 락) — verified 오탐은 gate 를 헐겁게 하므로 | 5 | fixed (2026-08-13) |
