...
|---|---|---|---|---|
| 1 | `scripts/guard-worktree-edit.js` — ①main 추적파일 편집에 `ask`(**auto 모드는 제외** — `7c977c8`, 2026-08-06) ②worktree 세션의 worktree 밖 편집 `deny` | worktree isolation 이 **모든 세션 타입**의 file edit + Bash 에 적용, main 파괴적 git checkout 차단 (v2.1.222 · 2026-08-04); `EnterWorktree` 가 `.claude/worktrees/` 밖 진입 시 확인 (v2.1.206 · 2026-07-09) | `watch` | 두 기능을 갈라 봐야 한다. **②는 네이티브 v2.1.222 와 같은 방향**이라 가장 유력한 `retire` 후보 — 다음 점검에서 네이티브 isolation 이 이 repo 의 hook 설정 없이도 동일 범위를 덮는지 실측할 것. **①은 방향이 반대**(네이티브=worktree 안에서 밖으로 새는 것 차단 / 자작=main 세션에서 편집을 *시작*하는 것 차단, [[worktree-per-task]])라 아직 대체 아니나, auto 모드에서 이미 스스로 꺼져 적용 면적이 줄었다 |
...
`retire` 판정: **0건** (이번 창 기준). 다음 점검의 1순위는 **1행 ②**(worktree 밖 편집 deny) — 네이티브 v2.1.222 와 방향이 같아 유일하게 실측만 남은 후보다.
