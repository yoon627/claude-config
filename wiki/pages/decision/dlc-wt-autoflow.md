...
이유: ① 사용자가 slug 승인을 마찰로 지목. ② 위험기반 기준상 worktree 생성은 로컬·비파괴·`/wt rm` 한 번으로 가역이라 확인 대상이 아니다 — 확인은 비가역·외부공개·파괴적에만. ③ 원래 근거였던 "이름 오타로 인한 오생성"은 승인이 아니라 **정보**로 막는다: wt request §4 가 base·stale·충돌 suffix·near-miss(단일 토큰이 기존 worktree 와 유사)와 `/wt rm <slug>` 되돌리기를 생성 직후 보고한다.
