...
## structural 파이프라인 (요지)
Setup → Explore → draft plan → plan-reviewer → TDD Red → 구현 → Green(목적 2개+ 면 단위 커밋) → code-reviewer+architecture-reviewer(병렬) → fix loop(≤2) → simplify 체크(메인 직접) → targeted 재리뷰 → 최종 검증 → **마무리(evidence gate → 판정 DONE/BLOCKED/NEEDS-HUMAN → plan 갱신 → 정식 완료 커밋(DONE 만) → commit-check → 알림 → Report)**. commit-check 는 미게시 커밋의 단위를 점검하고 승인 시 [[commit-restructure-plumbing-cas]] 방식으로 재구성한다. medium 이상에서 목적이 2개 이상이면 목적 단위로 중간 커밋하고 커밋된 단위의 후속 수정은 `fixup!` 으로 쌓아 commit-check 가 `fixup_of` 로 합친다(plan 파일은 마지막 커밋에만 — 앞 단위에 넣으면 3-way 충돌). 최종 검증은 격리 runner가 실행하고 메인이 판단. 커밋을 plan 갱신 뒤에 두는 이유는 [[comment-and-commit-policy]] 와 CLAUDE.md §8.
