...
## wt skill
`/wt <요청사항>` → slug 파생(확인 없음 — 근거는 [[dlc-wt-autoflow]]) → `.claude/worktrees/<name>/`에 prefix 없는 브랜치 생성 → main에서 `.env` 복사 → submodule heal + bootstrap → codegraph 인덱스 백그라운드 init → [[dlc-development-cycle]] 시작. `/wt <N>`·기존 이름은 이동, `?` 접두는 질문 모드.
