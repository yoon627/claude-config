...
- **커밋은 리뷰 가능한 하나의 목적 단위**: 아직 push 하지 않은 커밋이면 같은 작업의 후속 수정(리뷰 반영·오타·단순 실수)은 새 커밋으로 쌓지 말고 대상 커밋에 합치고, 목적이 섞인 커밋은 나눈다(이미 push 한 커밋은 고치지 않고 새 커밋 — force-push 금지). 메시지 형식은 그 repo 의 AGENTS/CLAUDE 커밋 절이 정본. 점검·재구성은 `commit-check` 스킬 — 미게시 커밋만, 사용자 승인 후, 코드 내용은 그대로(plumbing 재조립이라 hook 우회 `--no-verify` 가 아니다).
