...
SKILL 은 체크리스트 6항(문제·제약·완료기준·범위·산출물·제외) + `# Intent` 기록(medium 이상 항상) + 묶음 intent 스캔 + 분할 판정(medium 이상) + fail-safe 질문 + silent + trivial 예외 + ≤2 라운드만 인라인. 질문을 **어떻게 구성/우선순위** 하나:
- **질문 vs 추천 경계** — **무엇(요구·목표·산출물)이 빠졌으면 질문**, **요구는 있고 구현 방법만 갈리면 분석 후 추천**. 둘 다면 acceptance 를 바꾸는 축(what) 먼저.
- **질문 우선순위** — 질문이 여럿이면 **답이 설계·acceptance 를 바꾸는 것부터**, 사용자가 배치 질문에 부담을 보이면 한 번에 하나씩(인터뷰식). 체크리스트 6항 불변.
...
SKILL 은 2스텝 이름(재현 → 인과 사슬)·"재현 없이 고쳤다 금지"·대안 원인 반증 원칙을 인라인. CLAUDE.md §1(근본 원인·최소 3 Whys)의 dlc 구체화:
1. **재현**: 실패를 먼저 재현. 재현 없이 "고쳤다" 금지.
2. **인과 사슬**: 증상 → 직접 원인 → 근본 원인까지 증거로 확정. 원인이 불확실하면 다른 원인 후보를 반증한 근거를 함께 남긴다(첫 가설에 안주하지 않는다 — 후보 개수는 정하지 않는다). 증상만 누르는 수정(에러 무시·무의미 retry) 금지.
